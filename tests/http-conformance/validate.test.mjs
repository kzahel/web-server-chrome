...
const corpus = JSON.parse(
  await readFile(new URL("corpus-v1.json", root), "utf8"),
);
const schema = JSON.parse(
  await readFile(new URL("schema-v1.json", root), "utf8"),
);

test("corpus has the declared versioned schema", () => {
  assert.equal(
    schema.$id,
    "https://ok200.app/schemas/http-conformance-v1.json",
  );
...
test("every case is uniquely named and claimed or explicitly excluded by every runtime", () => {
  ...
  for (const entry of corpus.cases) {
    ...
    ids.add(entry.id);
    assert.ok(
      corpus.configurations[entry.configuration],
      `${entry.id}: unknown configuration`,
    );
    assert.ok(
      ["request", "oversizedHead", "concurrency", "restart"].includes(
        entry.kind,
      ),
    );
    assert.ok(
      Array.isArray(entry.expect.statuses) && entry.expect.statuses.length > 0,
    );
    ...
    for (const runtime of corpus.runtimes) {
      ...
      assert.notEqual(
        claimed,
        Boolean(exclusion),
        `${entry.id}: ${runtime} must be claimed or excluded`,
      );
    }
    assert.ok(
      entry.claims.every((runtime) => corpus.runtimes.includes(runtime)),
    );
    assert.ok(
      Object.keys(entry.exclusions).every((runtime) =>
        corpus.runtimes.includes(runtime),
      ),
    );

    if (entry.kind === "request" || entry.kind === "concurrency")
      assert.ok(entry.request);
    if (entry.kind === "concurrency")
      assert.ok(entry.concurrency >= 2 && entry.concurrency <= 32);
    if (entry.kind === "oversizedHead")
      assert.ok(entry.oversizedHeaderBytes > 16384);
  }
